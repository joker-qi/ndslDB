...
namespace leveldb {
  ...
  DBImpl::~DBImpl() {
    ...
    while (bg_compaction_scheduled_ || bg_clean_scheduled_) {
      bg_cv_.Wait();
    }
    if(drop_count_ > 0)
    {
  ...
  Status DBImpl::Recover(VersionEdit* edit, bool *save_manifest) {
    ...
    s = versions_->Recover(save_manifest, log_number, head_pos, vloginfo,
        recover_clean_vlog_number_, recover_clean_pos_);
  ...
  Status DBImpl::RecoverLogFile(uint64_t log_number, bool last_log,
    ...
    else
    {
      ...
      status = options_.env->NewAppendableFile(vlog_name, &vlfile);
      vlogfile_ = vlfile;
  ...
  void DBImpl::MaybeScheduleClean(bool isManuaClean)
  {
    ...
    else if(vlog_manager_.HasVlogToClean() || isManuaClean)
    {
      ...
      if(!isManuaClean)
      {
        //    env_->Schedule(&DBImpl::BGClean, this);
