...
namespace leveldb {
  ...
  class DBImpl : public DB {
    public:
      ...
      // NOTE: we dont support snapshot anymore
    ...
    private:
      ...
      uint64_t logfile_number_;      // Current vlog number
      log::VWriter* vlog_;           // vlog Writer
      WritableFile* vlogfile_;       // vlog reader
      uint64_t vlog_head_;           // This where we start to append kv data to vlog
      uint64_t check_point_;         // This where the recover process starts
      uint64_t check_log_;           // Which vlog to start recover
      // FIXME: Actuall we need all kv information instead of just drop_count_
      // we will fix later.
      uint64_t drop_count_;          // Remember drop count of each vlog
